			...
			if (!$elm$core$List$length(ids)) {
				...
				var _v0 = $elm$core$List$unzip(
					A2(
						$elm$core$List$map,
						function (id) {
							var _v1 = A2($elm_community$graph$Graph$get, id, c.pins);
							if (_v1.$ === 'Just') {
								var ctx = _v1.a;
								var nextIdList = A2(
									$elm$core$List$map,
									$elm$core$Tuple$first,
									$elm_community$intdict$IntDict$toList(ctx.outgoing));
								var inputIds = A2(
									$elm$core$List$map,
									$elm$core$Tuple$first,
									$elm_community$intdict$IntDict$toList(ctx.incoming));
								var newValue = function () {
									if (inputIds.b) {
										var headId = inputIds.a;
										var tailIds = inputIds.b;
										var firstValue = A2($author$project$Main$getValue, headId, c);
										if (!tailIds.b) {
											return firstValue;
										} else {
											var secondId = tailIds.a;
											return A2(
												$author$project$Main$nand,
												firstValue,
												A2($author$project$Main$getValue, secondId, c));
										}
									} else {
										return ctx.node.label;
									}
								}();
								return _Utils_Tuple2(newValue, nextIdList);
							} else {
	...
	var startIds = A2(
		...
		function (inputId) {
			var nextId = A2(
				...
				A2($elm_community$graph$Graph$get, inputId, c.pins));
			return A2($elm$core$Maybe$withDefault, 0, nextId);
		},
		c.inputPins);
...
var $author$project$Main$showCircuit = function (c) {
	...
	var nodeLabelToAttributes = function (label) {
